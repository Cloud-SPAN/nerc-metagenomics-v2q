...
![Analysis flow diagram that shows the steps: Sequence reads, Quality control and assembly.](/images/03_short_analysis_flowchart_short_asm.png){fig-align="right" width="325" height="316" fig-alt="Analysis flow diagram that shows the steps: 
...
![Metagenomic flow diagram with the steps raw reads, assembly and polishing and binning.](/images/03_genomics_v_metagenomics.png){width="775" height="717" fig-align="center" fig-alt="Metagenomic flow diagram with the steps raw reads, assembly and polishing and binning."}
...
Make sure you're still logged into your cloud instance. If you can't remember how to log on, visit [the instructions from earlier today](./01-QC-quality-raw-reads.qmd#getting-started).
:::
...
![.](/images/methods_guppy_version.png){width="700"}
...
First let's make sure we're in the `results` directory, and make a new directory called `assembly` for the Flye output.
...
The final thing to add to our `flye` command is "redirection": `&> results/assembly/flye_output.txt.txt` will send any output that would be sent to the terminal to a file called `flye_output.txt` (in `results/assembly`) instead.
...
[2023-02-24 14:41:27] root: INFO: Final assembly: /home/csuser/cs_course/results/assembly/assembly.fasta
```
